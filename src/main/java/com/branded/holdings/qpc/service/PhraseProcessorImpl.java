...
public class PhraseProcessorImpl implements  PhraseProcessor, ApplicationContextAware {
	...
	public void setStoplists(String stoplists) {		
		getStoplist(stoplists);
		initializeMalletInstanceList();
	}
	
	private void initializeMalletInstanceList(){
		ArrayList<Pipe> pipeList = new ArrayList<Pipe>();

		// Pipes: lowercase, tokenize, remove stopwords, map to features
		pipeList.add( new CharSequenceLowercase() );
		pipeList.add( new CharSequence2TokenSequence(Pattern.compile("\\p{L}[\\p{L}\\p{P}]+\\p{L}")) );
		pipeList.add( new TokenSequenceRemoveStopwords(stopListsFile, "UTF-8", false, false, false) );
		pipeList.add( new TokenSequence2FeatureSequence() );

		instances = new InstanceList (new SerialPipes(pipeList));
	}
	
	private void getStoplist(String stoplists){
		try {
			
			stopListsFile =ctx.getResource("classpath:"+stoplists).getFile();
...
//			stoplists = lines.toString();
	...
	private void getTopics(Phrase phrase) throws IOException{
		StringBuffer buffer = new StringBuffer(phrase.getDescription());
		
		
		Reader fileReader = new InputStreamReader(new ByteArrayInputStream(buffer.toString().getBytes()), "UTF-8");
		...
		//  the second is 
		ParallelTopicModel model = new ParallelTopicModel(numTopics, 10.0, 0.1);
